...
export default function Illustration40X() {
  return (
    <div className="relative flex h-[400px] w-full max-w-[450px] items-center justify-center">
      ...
      {/* Nodes */}
      <motion.div className="absolute left-[80px] top-[80px] h-10 w-10 rounded-xl bg-articleBg border border-explorerBorder shadow-[0_0_20px_var(--accentColor)] flex items-center justify-center animate-pulse">
        <div className="h-3 w-3 bg-accentColor rounded-full" />
      </motion.div>
      <motion.div className="absolute right-[80px] top-[80px] h-14 w-14 rounded-full bg-mainBg border-2 border-accentColor/50 flex items-center justify-center" animate={{ scale: [1, 1.2, 1] }} transition={{ duration: 2, repeat: Infinity }}>
        <span className="text-xs text-accentColor">API</span>
      </motion.div>
      <motion.div className="absolute left-[30px] bottom-[80px] h-12 w-12 rounded-lg bg-accentColor/20 border border-accentColor flex items-center justify-center backdrop-blur-md">
        <span className="text-xs text-white">DB</span>
      </motion.div>
      <motion.div className="absolute right-[50px] bottom-[50px] h-16 w-16 rounded-full bg-articleBg border border-explorerBorder flex items-center justify-center shadow-xl">
      ...
      >
        <div className="h-20 w-20 rounded-full border-[4px] border-dashed border-accentColor/50" />
      </motion.div>
    </div>
  );
}
